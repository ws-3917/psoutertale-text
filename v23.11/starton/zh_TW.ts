...
const text = {
   a_starton: {
      telescope1: () => [
         ...(SAVE.data.b.svr ? [] : [ "<32>{#p/basic}* 標準規格的CITADEL長焦望遠鏡，\n  251X年前後製成。" ]),
         choicer.create("* （使用望遠鏡？）", "是", "否")
      ],
      telescopeMeetup1: [ "<25>{#p/kidd}{#f/2}* 你在看星星嗎？" ],
      telescopeMeetup2: [
         "<25>{#p/kidd}{#f/1}* 喲... 我打賭你肯定看到了很酷的東西。",
         "<25>{#f/7}* 上次我用望遠鏡的時候，我甚至看到了一顆超新星！"
      ],
      telescopeMeetup3: [
         "<25>{#p/kidd}{#f/3}* 給。\n* 拿上這個。",
         "<32>{#s/equip}{#p/human}* (高級會員券被添加到了你的鑰匙圈。)",
         "<25>{#p/kidd}{#f/7}* 現在你可以使用任何望遠鏡了，連“最高級”的都能用！",
         "<25>{#f/1}* 那個矮骷髏之前給了我好多這個。",
         "<25>{#f/2}* 他甚至花了很多錢送了我一些數字藏品...",
         "<25>{#f/1}* 我猜他真的很喜歡我，哈哈。"
      ],
      telescopeMeetup4: [
      ...
      telescope2: () =>
         SAVE.data.b.svr
            ...
            : ((SAVE.data.b.s_state_chargazer = true),
              [
                 "<32>{#p/basic}* ...",
                 "<32>* Asriel和我曾經就有一架這樣的望遠鏡。",
                 "<32>* 我們隨意指向天空，希望看到什麼激動人心的東西...",
                 "<32>* ... 我們從來沒看到過那麼多。",
                 "<32>* 雖然這樣，但是這對他來說似乎並不重要...",
                 "<32>* 我專注地向外看的時候，他僅僅是開心地和我在一起。",
                 "<32>* ...",
                 "<32>{#p/human}* (你聽到一聲嘆息。)",
                 "<32>{#p/basic}* ... 呃，現在回到剛才的事吧。"
              ]),
      ...
      maze1: () =>
         world.edgy
            ? [
                 "<25>{#p/sans}{#f/0}* 歡迎回來。",
                 "<25>{#p/sans}{#f/3}* 很可惜papyrus不能到場，因為…",
                 "<25>{#p/sans}{#f/2}* 他在這個謎題上花了很長一段時間。",
                 "<25>{#p/sans}{#f/0}* 但是這並沒關係。",
                 "<25>{#p/sans}{#f/0}* 我向他保證過了我會給你展示這個的，所以我激活它了。"
              ]
            : [
      ...
      maze5: () =>
         world.edgy
            ? [
                 ...
                 "<25>{#p/sans}* 很簡單，對吧？",
                 "<25>{#p/sans}{#f/3}* 雖然說，我親自測試過這個機關，我還是不得不說…",
                 "<25>{#p/sans}{#f/2}* 它可沒有看上去的那樣簡單。"
              ]
            : [
      ...
      sans1: [
         "<99>{#p/darksans}{#i/4}* {@spacing:4,0}{#x4}人類。",
         "<99>* {@spacing:4,0}{#x4}難道你不知道{@spacing:}\n  {@spacing:4,0}{#x4}怎麼和新朋友{@spacing:}\n  {@spacing:4,0}{#x4}打招呼嗎？",
         "<99>* {@spacing:4,0}{#x4}轉過身來{@spacing:}\n  {@spacing:4,0}{#x4}和我握手。"
      ],
      sans2: () => [
         ...(world.edgy
            ? [
   ...
   n_shop_blook: {
      ...
      talk: () =>
         SAVE.data.n.plot === 72
            ? [ "哈拉", "What Happened", SAVE.data.s.name || systemsText.general.mystery1, "The Future", "離開" ]
            : [
                 "哈拉",
                 "Ghosts",
                 "Sanctuary",
