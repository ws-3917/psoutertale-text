...
export default {
   a_foundry: {
      ...
      tome5a: '<32>{#p/human}* （你凝神屏息，试着饶恕对方。）',
      tome5b: () =>
         world.meanie
            ? '<32>{#p/human}* （你凝神屏息，试着欺负对方。）'
            : SAVE.data.b.oops && world.flirt > 9
            ? '<32>{#p/human}* （你凝神屏息，试着向对方调情。）'
            : SAVE.data.b.oops
            ? '<32>{#p/human}* （你凝神屏息，试着与对方交友。）'
            : '<32>{#p/human}* （你凝神屏息，试着拥抱对方。）',
      tome5c: '<32>{#p/human}* （你凝神屏息，试着击杀对方。）',
      ...
      genotext: {
         ...
         asriel44: [ '<25>{#p/asriel2}{#f/13}* 呃，$(name)，你来带路吧。' ],
         asriel45: [
      ...
      spookydate8: [ "<32>{#p/basic}* 我听说你用烈焰考尔比\n  把酒吧给点燃了，\n  是这样吗？" ],
      spookydate9: [
         '<25>{#p/sans}{#f/3}* 嗯？\n* 不，那东西完全无害。',
         '<25>{#f/2}* 能让这酒吧燃起来的\n  只有我的招牌幽默笑话。'
      ],
   ...
   i_epiphany: {
      ...
      use: () =>
         battler.active
            ...
            : [ '<32>{#p/human}* (You activated The Epiphany.)', '<32>{#p/human}* （战斗外使用无效。）' ]
   },
   i_astrofood: {
      battle: {
         description: '牙口不好的别吃。',
   ...
   i_tea: {
      ...
      use: () => [
         ...
         battler.active ? '<32>{#p/story}* 你的移速提升了1点！' : '<32>{#p/human}* （战斗外使用无效。）'
      ]
   },
   i_tzn: {
      battle: {
   ...
   n_shop_tem: {
      ...
      talk: () => [
         ...
         SAVE.data.n.plot === 72 ? 'Your Future' : SAVE.data.b.colleg ? '关于Temmie盔甲' : '介绍下自己',
         SAVE.data.n.plot === 72 ? 'Temmie Secrets' : 'Temmie的历史',
         '关于这个商店',
         '离开'
      ],
