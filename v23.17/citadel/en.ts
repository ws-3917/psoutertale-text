   ...
   b_opponent_final: {
      ...
      friend67: (unique: string[]) => [
         ...
         ...(unique.find(k => k.startsWith('light')) && unique.find(k => k.startsWith('dark')) // NO-TRANSLATE
            ? [
                 // TODO: twinkly comments on past neutral endings
                 // "oh, come ON. i think you know the answer."
                 // "after all, you're the one who [did multiple neutrals]"
                 // "you know what it's like, don't you?"
                 // playing with the world, like it's all just a game?
                 // number of unique neutrals: `SAVE.flag.s.neutral_unique.split(',').filter(x => x).length`
              ]
            : [
