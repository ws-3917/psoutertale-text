...
export default {
   a_foundry: {
      ...
      blookdate6b: [ '<32>{#p/napstablook}* 喔......................', "<32>* i'll just be outside then" ],
      blookdate7: [
      ...
      genotext: {
         ...
         asriel44: [ '<25>{#p/asriel2}{#f/13}* 呃，$(name)，你來帶路吧。' ],
         asriel45: [
      ...
      spookydate8: [ "<32>{#p/basic}* 我聽說你用烈焰考爾比\n  把酒吧給點燃了，\n  是這樣嗎？" ],
      spookydate9: [
         '<25>{#p/sans}{#f/3}* 嗯？\n* 不，那東西完全無害。',
         '<25>{#f/2}* 能讓這酒吧燃起來的\n  只有我的招牌幽默笑話。'
      ],
   ...
   b_opponent_doge: {
      ...
      act_flirt: () => [
         ...(dogecon() || world.goatbro
            ...
            ? [ '<32>{#p/human}* （你向Doge調情。）', '<32>{#p/basic}* Doge smiles in return.' ]
            ...
            : world.flirt < 10
            ? [ '<32>{#p/human}* （你向Doge調情。）', "<32>{#p/basic}* Doge doesn't react in any strong way." ]
   ...
   b_opponent_muffet: {
      ...
      appeaseTextPost: [ "<32>{#p/human}* （但Muffet不需要被再次安撫。）" ],
   ...
   b_opponent_undyne: {
      ...
      randStatus4: () =>
         respecc() ? [ '<32>{#p/story}* Undyne滿懷激情地跳來跳去。' ] : [ '<32>{#p/story}* Undyne焦躁的跳來跳去。' ],
      randStatus5: () =>
         respecc()
   ...
   i_epiphany: {
      ...
      use: () =>
         battler.active
            ...
            : [ '<32>{#p/human}* (You activated The Epiphany.)', '<32>{#p/human}* （戰鬥外使用無效。）' ]
   },
   i_astrofood: {
      battle: {
         description: '牙口不好的別吃。',
   ...
   i_tea: {
      ...
      use: () => [
         ...
         battler.active ? '<32>{#p/story}* 你的移速提升了1點！' : '<32>{#p/human}* （戰鬥外使用無效。）'
      ]
   },
   i_tzn: {
      battle: {
